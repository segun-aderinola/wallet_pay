...
<div class="col-sm-12 tabl">
  <h4 align="center"> UPDATE RECORD OF INDIVIDUAL</h4><br>
 ...
 <?php
      include_once('../connection.php');
      $edit_h=$_GET['edit_h'];
      $sql = "SELECT people.*,individual.houseType
FROM people
INNER JOIN individual ON people.serial_no='$edit_h' AND individual.individual_id='$edit_h'";
$result = mysqli_query($conn, $sql);
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      ?>
       <form id='form1'  action='update_individual.php?edit_he=<?php echo $edit_h?>' method='post'>
    
      <tr>
        <td> <label for="name">Name:</label></td>
        <td><input type="name" minlength='3' id='name' class="form-control"  placeholder="Enter Name" name="name"  pattern="[A-Za-z\s]+" value='<?php echo $row['name'];?>' required><span class='form_error' id='error_name'></span></td>
        <td> <label for="father_name">Father Name:</label></td>
        <td>  <input type="text"  minlength='3' id='fname' class="form-control"  placeholder="Enter father_name" name="father_name" pattern="[A-Za-z\s]*" value='<?php echo $row['fatherName'];?>' required ><span class='form_error' id='error_fname'></span></td>
      </tr>
    <tr><td><br></td></tr>
      <tr>
        <td> <label for="Cnic">Cnic:</label></td>
        <td> <input type="text" class="form-control" id='cnic' placeholder="Enter Cnic with e.g 15302-6769504-1" maxlength='15' name="cnic" value='<?php echo $row['cnic'];?>' pattern="[1-6]{1}\d{4}[\-]\d{7}[\-]\d{1}"><span class='form_error' id='error_cnic'></span></td>
...
<div class="radio">
  <label><input type="radio" name="gender" value='other'>Other</label>
      ...
      <tr>
        ...
        <td><input type='date' name='birthday' id='date' value=<?php echo $row['age'];?>><span class='form_error' id='error_date'></span></td>
  ...
  <td  class='text-center'><label for="job">House type</label></td>
    ...
    <option value="have house">Have House</option>
  ...
  <tr>
    ...
    <tr>
      <td colspan='4' align="center"><input  class='btn btn-block btn-success' name="submit" class='form-control bg-success text-success ' type="submit" value="UPDATE INDIVIDUAL"></td>
    </tr>
