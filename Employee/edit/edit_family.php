...
<div class="col-sm-12 tabl">
  <h4 align="center"> UPDATE RECORD of  FAMILY</h4><br>
 ...
 <?php
      include_once('../connection.php');
      $edit_h=$_GET['edit_h'];
      $sql = "SELECT people.*,family.relation_head,family.head_id
FROM people
INNER JOIN family ON people.serial_no='$edit_h' AND family.family_id='$edit_h'";
$result = mysqli_query($conn, $sql);
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      ?>
       <form id='form1' action='update_family.php?edit_he=<?php echo $edit_h?>' method='post'>
     <input type='hidden' name='head_id' value="<?php echo $row['head_id'];?>">
      <tr>
        <td> <label for="name">Name:</label></td>
        <td><input type="name" minlength='3' class="form-control" id='name' placeholder="Enter Name" name="name"  pattern="[A-Za-z\s]+" value='<?php echo $row['name'];?>' required><span class='form_error' id='error_name'></td>
        <td> <label for="father_name">Father Name:</label></td>
        <td>  <input minlength='3' type="text" class="form-control" id='fname'  placeholder="Enter father_name" name="father_name" pattern="[A-Za-z\s]*" value='<?php echo $row['fatherName'];?>' required ><span class='form_error' id='error_fname'></td>
      </tr>
    <tr><td><br></td></tr>
      <tr>
        <td> <label for="Cnic">Cnic:</label></td>
        <td> <input type="text" class="form-control" id='cnic' placeholder="Enter Cnic with e.g 15302-6769504-1" maxlength='15' name="cnic" value='<?php echo $row['cnic'];?>' pattern="[1-6]{1}\d{4}[\-]\d{7}[\-]\d{1}"><span class='form_error' id='error_cnic'></td>
...
<div class="radio">
  <label><input type="radio" name="gender" value='other'>Other</label>
      ...
      <tr>
        ...
        <td><input type='date' name='birthday' id='date' value=<?php echo $row['age'];?>><span class='form_error' id='error_date'></td>
  ...
  <td  class='text-center'><label for="relation">Relation to head</label></td>
   <td><select name="relation" class='form-control'>
    <option value="<?php echo $row['relation_head'];?>"><?php echo $row['relation_head'];}}?></option>
  ...
  <tr>
    ...
    <tr>
      <td colspan='4' align="center"><input  class='btn btn-block btn-success' name="submit" class='form-control bg-success text-success ' type="submit" value="UPDATE FAMILY"></td>
    </tr>
