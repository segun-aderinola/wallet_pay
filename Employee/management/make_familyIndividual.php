...
echo " <div class='row'>
  ...
  if(mysqli_num_rows($res)>0)
  {
  	...
  	if(mysqli_num_rows($result)>0){
  	  	...
  	  	$updat="INSERT INTO family(head_id,family_id,relation_head) VALUES('$id','$change_id','$relation')";
  	  	 if(mysqli_query($conn,$updat))
  	  	{
          
         $del="DELETE FROM individual WHERE individual_id='$change_id'";
        if(mysqli_query($conn,$del)){
          
  	  		echo "<script>alert('person added to head Successful')</script>"; 
     echo "<script>
setTimeout(function() {
  window.location.href = '../people_view.php';
  		...
  		echo "<br><font color='red'>This cnic not belong to Head</font>";
  	}

  }
  else
  {
  	echo "<br><font color='red'>This cnic is not exist in record</font>";
  }
